...
class LM(AbstractLanguageChecker):
	def __init__(self, model_name_or_path="gpt2-l"):
		...
		self.enc = GPT2Tokenizer.from_pretrained(model_name_or_path,cache_dir='./models1')
		self.model = GPT2LMHeadModel.from_pretrained(model_name_or_path,cache_dir='./models1')
...
# def compute_metrics(preds, labels, y_true, y_scores, model):
#     # Compute metrics, ensuring we handle the case where only one class exists
#     precision, recall, fscore, support = score(labels, preds)

#     # Attempt to calculate AUC only if there are at least two distinct label values
#     auc_test = None
...
#             auc_test = None  # AUC is set to None if there's an error in calculation

#     # Prepare the metrics dictionary safely checking for index errors
#     metrics = {
#         "model": model,
#         "AUC": auc_test,
#         "acc": np.mean(preds == labels),
#         "precision_machine": precision[0],  # First class present in the labels
#         "recall_machine": recall[0],
#         "fscore_machine": fscore[0],
#         "support_machine": float(support[0]),
#         "precision_human": precision[1] if len(precision) > 1 else None,
#         "recall_human": recall[1] if len(recall) > 1 else None,
#         "fscore_human": fscore[1] if len(fscore) > 1 else None,
#         "support_human": float(support[1]) if len(support) > 1 else None
#     }
