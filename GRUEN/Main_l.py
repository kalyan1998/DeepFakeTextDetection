...
@Language.component("wmd_hook")
def wmd_component(doc):
...
def main():
    ...
    def preprocess_candidates(candidates):
        ...
        for candidate_i in candidates:
            ...
            for sentence_i in sentences:
                if len(sentence_i.translate(str.maketrans('', '', string.punctuation)).split()) > 1:
    ...
    def get_grammaticality_score(processed_candidates):
        ...
        grammaticality_score = [max(0, x / 8.0 + 0.5) for x in grammaticality_score]
    ...
    def get_focus_score(all_summary):
        def compute_sentence_similarity():
            ...
            nlp.add_pipe("wmd_hook", last=True)
            all_score = []
            for i in range(len(all_summary)):
    ...
    parser.add_argument("--cola_dir", default=None, type=str, required=True)
    parser.add_argument("--cache_dir", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--discriminator_type", type=str, required=True)
