...
def download_nltk_resources():
    resources = ['omw-1.4', 'averaged_perceptron_tagger', 'universal_tagset']
    for resource in resources:
        try:
            if not nltk.data.find(resource):
                nltk.download(resource, download_dir='/home/sdiwaka/nltk_data')
        except LookupError:
            nltk.download(resource, download_dir='/home/sdiwaka/nltk_data')
        except Exception as e:
            print(f"Failed to download {resource}: {e}")

nltk.data.path.append('/home/sdiwaka/nltk_data')
...
class USE(object):
    ...
    def semantic_sim(self, sents1, sents2):
        # Compute embeddings for both sets of sentences
        embeddings1 = self.model(sents1)
        embeddings2 = self.model(sents2)

        # Normalize the embeddings
        sts_encode1 = tf.nn.l2_normalize(embeddings1, axis=1)
        sts_encode2 = tf.nn.l2_normalize(embeddings2, axis=1)
        ...
        sim_scores_evaluated = self.session.run(sim_scores)
        return sim_scores_evaluated
