...
class LM(AbstractLanguageChecker):
	def __init__(self, model_name_or_path="gpt2-l"):
		...
		self.enc = GPT2Tokenizer.from_pretrained(model_name_or_path,cache_dir='./models')
		self.model = GPT2LMHeadModel.from_pretrained(model_name_or_path,cache_dir='./models')
...
class BERTLM(AbstractLanguageChecker):
	def __init__(self, model_name_or_path="bert-large-cased"):
		...
		self.tokenizer = BertTokenizer.from_pretrained(
			model_name_or_path,cache_dir='./models',
			do_lower_case=False)
...
#         auc_test = None  # Set AUC to None if an error occurs
...
# def compute_metrics(preds, labels, y_true, y_scores, model):
#     # If there's only one class, adjust the scores computation accordingly
#     if len(np.unique(labels)) == 1:
#         print("Only one class present in y_true. Metrics calculation will be adjusted.")
#         accuracy = np.mean(preds == labels)
#         return {
#             "model": model,
#             "AUC": None,  # AUC is not defined for one class
#             "acc": accuracy,
#             "precision_machine": None,
#             "recall_machine": None,
#             "fscore_machine": None,
#             "support_machine": len(labels),
#             "precision_human": None,
#             "recall_human": None,
#             "fscore_human": None,
#             "support_human": 0,  # No human class present
#         }

#     # If both classes are present, proceed as normal
#     precision, recall, fscore, support = score(labels, preds)
#     try:
#         auc_test = roc_auc_score(y_true, y_scores[:, 1])  # Ensure y_scores for the positive class is used
#     except ValueError as e:
#         print(f"Warning: {str(e)} - AUC score will be set to None")
#         auc_test = None  # Set AUC to None if an error occurs
...
#         "fscore_machine": fscore[1],
#         "support_machine": float(support[1]),
#         "precision_human": precision[0] if len(precision) > 1 else None,
#         "recall_human": recall[0] if len(recall) > 1 else None,
#         "fscore_human": fscore[0] if len(fscore) > 1 else None,
#         "support_human": float(support[0]) if len(support) > 1 else 0,
#     }
